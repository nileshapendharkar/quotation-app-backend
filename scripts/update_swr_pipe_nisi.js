...
const newBaseName = "SWR PIPES NISI RINGFIT (3 MTR)";

const sizeMapping = [
  { size: '2 1/2" (75MM) 4KG - 3MTR', code: 'FG-401425', aliases: ['2 1/2', '75MM 4KG', '2 1/2 75MM 4KG', '2 1/2" (75MM) 4KG', '2 1/2" (75MM) 4KG - 3MTR'] },
  { size: '3" (90MM) 4KG - 3MTR', code: 'FG-401426', aliases: ['3', '90MM 4KG', '3 90MM 4KG', '3" (90MM) 4KG', '3" (90MM) 4KG - 3MTR'] },
  { size: '4" (110MM) 4KG - 3MTR', code: 'FG-401427', aliases: ['4', '110MM 4KG', '4 110MM 4KG', '4" (110MM) 4KG', '4" (110MM) 4KG - 3MTR'] },
  { size: '6" (160MM) 4KG - 3MTR', code: 'FG-401428', aliases: ['6', '160MM 4KG', '6 160MM 4KG', '6" (160MM) 4KG', '6" (160MM) 4KG - 3MTR'] }
];

// 1. Update db_data.json
const dbData = JSON.parse(fs.readFileSync(dbDataPath, 'utf8'));

const prod = dbData.products.find(p => p.id === 'prod_swr_2' || p.name === 'SWR RINGFIT - RINGFIT PIPE');
if (prod) {
  ...
  prod.description = `${newBaseName}. Available sizes: 2 1/2" (75MM) 4KG - 3MTR, 3" (90MM) 4KG - 3MTR, 4" (110MM) 4KG - 3MTR, 6" (160MM) 4KG - 3MTR.`;
}
...
exportData.forEach(row => {
  if (row["Base Product"] === "SWR RINGFIT - RINGFIT PIPE" || row["Product Name"].startsWith("SWR RINGFIT - RINGFIT PIPE (")) {
    ...
    row["Description"] = `${newBaseName}. Available sizes: 2 1/2" (75MM) 4KG - 3MTR, 3" (90MM) 4KG - 3MTR, 4" (110MM) 4KG - 3MTR, 6" (160MM) 4KG - 3MTR.`;
    exportCount++;
  }
});
